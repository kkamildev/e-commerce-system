...
export const createAdminValidator = [
    ...
    body("password").exists({checkFalsy:true}).withMessage("email is required").isLength({max:72}).withMessage("Maximum length 72")
]
...
export const createUserValidator = [
    ...
    body("email").trim().exists({checkFalsy:true}).withMessage("email is required").isLength({max:75}).withMessage("Maximum length 75").isEmail().withMessage("Must be a email"),
    body("password").exists({checkFalsy:true}).withMessage("email is required").isLength({max:72}).withMessage("Maximum length 72"),
    body("role").trim().exists({checkFalsy:true}).withMessage("role is required").isIn(["Admin", "Product_manager", "Sales_manager", "Producer"]).withMessage("role is not allowed")
]
...
export const updateUserPasswordValidator = [
    body("id").trim().exists({checkFalsy:true}).withMessage("id is required"),
    body("newPassword").exists({checkFalsy:true}).withMessage("email is required").isLength({max:72}).withMessage("Maximum length 72"),
]
